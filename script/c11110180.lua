...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_FUSION_SUMMON+CATEGORY_DISABLE)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg1 or (ce~=nil and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)) then
		local mg=mg1:Clone()
		if mg2 then mg:Merge(mg2) end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,s.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,mg,mf,chkf)
		local tc=g:GetFirst()
		if tc then
			local mat1=nil
			if tc:IsSetCard(0x9b) or not mf then
				mat1=Duel.SelectFusionMaterial(tp,tc,mg,nil,chkf)
				tc:SetMaterial(mat1)
				local mat_melodious_count = mat1:FilterCount(Card.IsSetCard,nil,0x9b)
				...
				if Duel.SpecialSummon(tc,SUMMON_TYPE_FUSION,tp,tp,false,false,POS_FACEUP)~=0 then
					...
					if mat_melodious_count>0 and #ng>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
						Duel.BreakEffect()
						Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_NEGATE)
						local sg=ng:Select(tp,1,mat_melodious_count,nil)
	...
	if c:IsRelateToEffect(e) then
		...
		if Duel.SendtoDeck(g,nil,SEQ_DECKBOTTOM,REASON_EFFECT)~=0 then
			...
			if og:GetCount()>1 then
				Duel.SortDeckbottom(tp,tp,og:GetCount())
			end
		end
	end
end
