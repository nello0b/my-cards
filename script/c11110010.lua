...
function s.bttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local ct=math.min(2,Duel.GetMatchingGroupCount(s.opfilter,tp,0,LOCATION_ONFIELD,nil,e),
		Duel.GetMatchingGroupCount(s.gyfilter,tp,LOCATION_GRAVE,0,nil,e))
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g1=Duel.SelectTarget(tp,s.gyfilter,tp,LOCATION_GRAVE,0,1,ct,nil)
	local ect=g1:GetCount()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g2=Duel.SelectTarget(tp,s.opfilter,tp,0,LOCATION_ONFIELD,ect,ect,nil,e)
